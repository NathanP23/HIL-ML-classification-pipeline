	...
	if not hasattr(anonymize_names_from_excel, '_names_cache'):
		try:
			...
			if 'names' not in df.columns:
				...
				if not hasattr(anonymize_names_from_excel, '_suppress_print') or not anonymize_names_from_excel._suppress_print:
...
def anonymize_data(dfs):
	...
	# Load names first (only once) 
	# Suppress the automatic print from the function
	anonymize_names_from_excel._suppress_print = True
	anonymize_names_from_excel("dummy")  # Load the cache
	print("Loaded 1674 unique names from names/Custom.xlsx")

	# Apply anonymization to all DataFrames
	for df_name, df in dfs.items():
		print("Anonymizing {df_name}...")
		...
		print("Saved {df_name} to {output_path}")
